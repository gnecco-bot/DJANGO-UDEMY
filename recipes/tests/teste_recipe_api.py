...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    def test_recipe_api_list_loads_recipes_by_category_id(self):
        ...
        # We should only see recipes froom the wanted category
        self.assertEqual(
            len(response.data.get('results')), # type: ignore
            9
        )
    ...
    def test_recipe_api_list_logged_user_can_create_a_recipe(self):
        ...
        recipe_raw_data['preparation_time'] = 2
    ...
    def test_recipe_api_list_logged_user_cant_update_a_recipe_owned_by_another_user(self):
        ...
        # Assertion (Afirmação)
        # Another user cannot update the recipe, so the status code must be 403 Forbidden
        self.assertEqual(
            response.status_code, 
            403,
        )
